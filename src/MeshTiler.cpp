...
ctb::MeshTiler::prepareSettingsOfTile(MeshTile *terrainTile, GDALDataset *dataset, const TileCoordinate &coord, float *rasterHeights, ctb::i_tile tileSizeX, ctb::i_tile tileSizeY) const {
  ...
  // generate mesh water mask
  // unsigned int HEIGHTMAP_SIZE = tileSizeX * tileSizeY;
  unsigned int HEIGHTMAP_TILE_SIZE = 65;
  unsigned int WATERMASK_TILE_SIZE = 256;
  
  bool isWater = true;
  bool isLand = true;

  //unsigned char waterMask[65536] = { 0 };
  //for(unsigned int i = 0; i < HEIGHTMAP_TILE_SIZE; i++){
  //  for (unsigned int j = 0; j < HEIGHTMAP_TILE_SIZE; j++){
  //    
  //    float h = rasterHeights[i * HEIGHTMAP_TILE_SIZE + j];
  //    if(h <= 0)
  //    {
  //      unsigned int mask_i = 4 * i;
  //      unsigned int mask_j = 4 * j; 
  //      for(size_t x = 0; x < 4; x++){
  //        for(size_t y = 0; y < 4; y++){
  //          waterMask[(mask_i + x) * WATERMASK_TILE_SIZE + mask_j + y] = 255;
  //        }
  //      }
  //      isLand = false;
  //    }
  ...
  cv::Mat src(HEIGHTMAP_TILE_SIZE, HEIGHTMAP_TILE_SIZE, CV_8UC1);
  src.setTo(0);
  for (unsigned int i = 0; i < HEIGHTMAP_TILE_SIZE; i++) {
      for (unsigned int j = 0; j < HEIGHTMAP_TILE_SIZE; j++) {
          float h = rasterHeights[i * HEIGHTMAP_TILE_SIZE + j];
          if (h <= 0) {
              src.at<unsigned char>(i, j) = (unsigned char)255;
              isLand = false;
          }
          else {
              isWater = false;
  ...
  cv::Mat dst;
  cv::resize(src, dst, cv::Size(WATERMASK_TILE_SIZE, WATERMASK_TILE_SIZE), cv::INTER_LINEAR);
  //unsigned char* waterMask = dst.data;

  tileMesh.isWater = isWater;
  tileMesh.isLand = isLand;
  if (!isWater && !isLand) {
      if (dst.isContinuous()) {
          tileMesh.waterMask.assign(dst.data, dst.data + dst.total() * dst.channels());
      }
      else {
          for (int i = 0; i < dst.rows; ++i) {
              tileMesh.waterMask.insert(tileMesh.waterMask.end(), dst.ptr<unsigned char>(i), dst.ptr<unsigned char>(i) + dst.cols * dst.channels());
          }
      }
  }
}
