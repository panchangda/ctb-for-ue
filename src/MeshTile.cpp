 ...
 */
void
MeshTile::writeFile(CTBOutputStream &ostream, bool writeVertexNormals, bool writeWaterMask) const {
  ...
  // # Write 'Water Mask' for rendering water effects:
  if(writeWaterMask && triangleCount > 0){
    unsigned char extensionId = 2;
    ostream.write(&extensionId, sizeof(unsigned char));

    if(!mMesh.isWater && !mMesh.isLand){
      int extensionLength = 65536; // 256 * 256 * 1 = 65536
      ostream.write(&extensionLength, sizeof(int));
      for(size_t i = 0; i < extensionLength; i++)
        ostream.write(&mMesh.waterMask[i], sizeof(unsigned char));
    }
    else{
      int extensionLength = 1;
      ostream.write(&extensionLength, sizeof(int));
      unsigned char mask = 0;
      if(mMesh.isWater) mask = 255;
      ostream.write(&mask, sizeof(unsigned char));
    }
  }
}
