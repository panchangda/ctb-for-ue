 ...
 */
bool
ctb::CTBFileTileSerializer::serializeTile(const ctb::MeshTile *tile, bool writeVertexNormals, bool writeWaterMask, bool gzip) {
  const TileCoordinate *coordinate = tile;
  const string filename = getTileFilename(coordinate, moutputDir, "terrain");
  const string temp_filename = concat(filename, ".tmp");
  if(gzip){
    CTBZFileOutputStream ostream(temp_filename.c_str());
    tile->writeFile(ostream, writeVertexNormals, writeWaterMask);
    ostream.close();
  }
  else{
