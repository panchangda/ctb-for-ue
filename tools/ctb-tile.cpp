...
public:
  ...
  static void 
    ...
    static_cast<TerrainBuild *>(Command::self(command))->gzip = true;
  }

  static void 
    setWaterMask(command_t* command) {
    static_cast<TerrainBuild *>(Command::self(command))->waterMask = true;
  }
  ...
  // compress terrain file  
  bool gzip;
  // generate water mamsk
  bool waterMask;
};
...
static void
buildMesh(MeshSerializer &serializer, const MeshTiler &tiler, TerrainBuild *command, TerrainMetadata *metadata, bool writeVertexNormals = false, bool writeWaterMask = false, bool gzip = false) {
  i_zoom startZoom = (command->startZoom < 0) ? tiler.maxZoomLevel() : command->startZoom,
    endZoom = (command->endZoom < 0) ? 0 : command->endZoom;
  ...
  while (!iter.exhausted()) {
    ...
    if (serializer.mustSerializeCoordinate(coordinate)) {
      ...
      serializer.serializeTile(tile, writeVertexNormals, writeWaterMask, gzip);
      delete tile;
    }
...
runTiler(const char *inputFilename, TerrainBuild *command, Grid *grid, TerrainMetadata *metadata) {
  ...
  try {
    ...
    if (command->metadata) {
      ...
      buildMesh(serializer, tiler, command, threadMetadata, command->vertexNormals, command->waterMask, command->gzip);
...
main(int argc, char *argv[]) {
  ...
  command.option("-W", "--water-mask", "Write Water-Mask for Water Rendering and Coastline Anti-aliasing, only for `Mesh` format", TerrainBuild::setWaterMask);
